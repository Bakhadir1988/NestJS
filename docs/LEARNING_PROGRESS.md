...
**Дата последнего занятия:** 07.02.2026

---

### Урок 11: CRUD для Задач и Глобальный Рефакторинг ID

- [x] **Практика: CRUD для "Задач (Tasks)".** Начата реализация самого вложенного ресурса в нашей иерархии.
  - Реализованы методы `create` и `findAll` для `Tasks`, включая всю необходимую логику в сервисе (проверка прав, целостности данных, сортировка).
  - Создан `TaskEntity` для сериализации ответов.
- [x] **Архитектура: Переход на CUID.** По вашей инициативе был поднят важный архитектурный вопрос о недостатках последовательных `Int` ID.
  - Проведен **глобальный рефакторинг** всей системы.
  - **Схема Prisma:** Все первичные (`id`) и внешние (`...Id`) ключи во всех моделях (`User`, `Board`, `Column`, `Task`, `BoardMembership`) были изменены с `Int` на `String @default(cuid())`.
  - **Миграция:** Успешно создана и применена новая миграция базы данных.
  - **Рефакторинг кода:** Обновлены все затронутые модули (`Users`, `Auth`, `Boards`, `Columns`, `Tasks`). Были убраны `ParseIntPipe` и изменены типы `id` на `string` во всех контроллерах, сервисах и DTO.
- [x] **Тестирование:** Проведено сквозное ручное тестирование всего API после рефакторинга, подтвердившее корректную работу системы с новыми строковыми идентификаторами.
- [ ] **Практическое задание:** Завершить реализацию CRUD для ресурса "Задачи (Tasks)" (`findOne`, `update`, `remove`).
