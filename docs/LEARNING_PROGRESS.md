...
**Дата последнего занятия:** 06.02.2026

---

### Урок 10: Вложенные ресурсы и Продвинутые Права Доступа (Phase 2)

- [x] **Практика: Система приглашений.** Завершено практическое задание из прошлого урока. Реализован эндпоинт `POST /boards/:id/members` для приглашения пользователей на доску с определенной ролью.
  - Создан `InviteUserDto` для валидации.
  - Реализована сложная бизнес-логика в сервисе, включая проверки прав приглашающего (`OWNER`), существования пользователя, само-приглашения и повторного приглашения.
- [x] **Практика: CRUD для вложенного ресурса "Колонки".** С нуля создан полный модуль для управления колонками.
  - Использован Nest CLI для генерации ресурса.
  - Настроен вложенный роутинг: `POST` и `GET` на `/boards/:boardId/columns`, `PATCH` и `DELETE` на `/boards/:boardId/columns/:columnId`.
- [x] **Продвинутая Логика и Рефакторинг:**
  - Реализована серверная логика для вычисления порядка (`order`) новых колонок.
  - Применен принцип DRY: повторяющиеся проверки прав доступа вынесены в приватные методы-хелперы (`checkAccess`, `getColumn`).
  - Реализован паттерн "Soft Delete" для колонок, чтобы избежать случайного удаления задач. Проведена миграция схемы и рефакторинг всех методов сервиса.
- [x] **Продвинутые концепции NestJS:**
  - Создан кастомный декоратор `@CurrentUser` для чистого получения пользователя в контроллерах.
  - Использован декоратор `@HttpCode(204)` для возврата правильного статуса при удалении.
  - Реализована **сериализация**: создан `ColumnEntity` для сокрытия полей (`deletedAt`, `boardId`) в ответах API.
- [ ] **Практическое задание:** Реализовать CRUD для ресурса "Задачи (Tasks)".
...
- [x] **Практическое задание:** Реализовать эндпоинт для приглашения пользователя на доску.
